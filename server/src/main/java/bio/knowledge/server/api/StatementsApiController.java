...
public class StatementsApiController implements StatementsApi {
    ...
    public ResponseEntity<List<Statement>> getStatements( @NotNull @ApiParam(value = "a set of [CURIE-encoded](https://www.w3.org/TR/curie/) identifiers of  'source' concepts possibly known to the beacon. Unknown CURIES should simply be ignored (silent match failure). ", required = true) @RequestParam(value = "s", required = true) List<String> s,
